...
def get_best_path(digraph, start, end, path, max_dist_outdoors, best_dist,
    ...
    ###raise an error if start or end node isn't in digraph
    ...
    if start == end:
        ...
        for WeightedEdge in digraph.get_edges_for_node(Node(start)):
            ...
            if str(child) not in path[0]:
                ...
                print('child=',child,', end=',end)

                if newPath != None:
                    (path[1], path[2]) = get_distance(digraph, path[0])
    ...
    for i in range(len(path)-1):
        ...
        for WeightedEdge in digraph.get_edges_for_node(Node(path[i])):
            ...
            if WeightedEdge.dest == Node(path[i+1]):
                ...
                t_d = t_d + WeightedEdge.total_distance
                t_o_d = t_o_d + WeightedEdge.outdoor_distance  
                
    print("get_distance called, t_d =",t_d,", t_o_d =,",t_o_d)
