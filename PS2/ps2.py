...
def get_best_path(digraph, start, end, path, max_dist_outdoors, best_dist,
    ...
    if not ( digraph.has_node(startNode) and digraph.has_node(endNode) ):
        ...
        ###recursively solve the rest of the path, from child node to the end node
    
        ###for all child WeightedEdges of start
        
        '''need to change from weighted edges to child nodes
        '''
        
        for WeightedEdge in digraph.get_edges_for_node(startNode):
            '''currently node returns a weighted edge, instead of a node''' 
            
            child = WeightedEdge.get_destination()
            
            ###prevent cycling 
            if str(child) not in path or path == []:
    
                
                ###update path to include new node, update total_distance, and total_outdoor_distance

                """
                ###Add edge distance to total path distance
                path[1] = path[1] + int(WeightedEdge.get_total_distance())
                                              
                ###Add edge outdoor distance to total outdoor distance
                path[2] = path[2] + int(WeightedEdge.get_outdoor_distance())
                ...
                newPath = get_best_path(digraph, child, end, path, 
                                            max_dist_outdoors, best_dist, best_path)
                    
                if newPath != None:
                    total_dist, total_outdoor_dist = get_distance(digraph, path)
...
def get_distance(digraph, path):
    ...
    for i in range(len(path)-1):
        for edge in digraph.edges[path[i]]:
            total_dist = total_dist + edge.get_total_distance
            total_outdoor_dist = total_outdoor_dist + edge.get_total_outdoor_distance()
