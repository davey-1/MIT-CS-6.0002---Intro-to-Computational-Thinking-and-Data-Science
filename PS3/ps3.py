...
# Name:David
# Collaborators (discussion): Sakar
# Time:
...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        self.tile[(x,y)] = self.tile[(x,y)] - capacity
    ...
    def get_num_cleaned_tiles(self):
        ...
        for i in range(self.width):
            for j in range(self.height):
                if self.is_tile_cleaned(i,j) == True:
                    nct += 1
    ...
    def is_position_in_room(self, pos):
        ...
        if pos.get_x() < self.width and pos.get_x() >= 0:
            if pos.get_y() < self.height and pos.get_y() >= 0:
                return True
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        pos1 = self.room.get_random_position()
        
        self.pos = pos1
    
        #initial direction set to a random float between 0 and 360    
...
class EmptyRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        Returns: True if pos is in the room, False otherwise.
        """
        #DS Note: check < vs <= if a problem arises
        if pos.get_x() < self.width and pos.get_x() >= 0:
            if pos.get_y() < self.height and pos.get_y() >= 0:
                return True
    ...
    def get_random_position(self):
        ...
        """
        rand_x = random.random() * float(self.width)
        rand_y = random.random() * float(self.height)
    ...
    def add_furniture_to_room(self):
        ...
        for i in range(f_bottom_left_x, f_bottom_left_x + furniture_width):
            for j in range(f_bottom_left_y, f_bottom_left_y + furniture_height):
                ...
                self.clean_tile_at_position(pos, self.get_dirt_amount(i,j))
                
        #clean tile when furnighint
                
        """ *** DS note: furniture is stored as a list of tiles
        furniture_tiles = [] , with a coordinate pair stored as each value
        of the list"""
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        #Calculate new pos if it moved striaght in its current direction @ given speed
        robot_pos = self.get_robot_position()
        direction = self.get_robot_direction()

        newPos = robot_pos.get_new_position(direction, self.speed)
        
        #If a valid position, move there and clean the tile at that position (check which tile to clean)
        ...
        #else new random direction (don't clean current tile or move to a different tile)
        else: 
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        #*be mindful of having the right actions at the right timesteps
        ...
        if isFaulty:
            ...
            #treat like StandardRobot, function copypasted from StandardRobot
            robot_pos = self.get_robot_position()
            direction = self.get_robot_direction()
            newPos = robot_pos.get_new_position(direction, self.speed)
            if self.room.is_position_valid(newPos) == True:
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                FaultyRobot)
    """
    #min_coverage in FurnishedRooms needs to 
    #be calculated based on: total tiles - furnished tiles, not total tiles
    ...
    for i in range(num_trials):
        ...
        #add trial times to the list
        trial_times.append(time_steps)                
        #print("Trial #:",i, "Timesteps =", time_steps)
              
    #average the number ot time steps needed to complete the room
    avg = sum(trial_times)/num_trials
    #print("average of",num_trials,"trials is",avg,"timesteps.")
