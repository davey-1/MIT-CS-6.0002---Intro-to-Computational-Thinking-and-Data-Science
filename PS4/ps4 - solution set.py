...
class Patient(object):
    ...
    def update(self):
        ...
        pd = len(surviving_bacteria) / self.max_pop       
        
        num_survived = len(surviving_bacteria)
        for i in range(num_survived):
            try:
                new_bacteria = self.bacteria[i].reproduce(pd)
...
def calc_pop_std(populations, t):
    ...
    mean = sum(trial_values) / len(trial_values)

    factor1 = []
    for value in trial_values:
        factor1.append((value - mean)**2)
    
    std = (sum(factor1) / len(trial_values))**0.5
    ...
    def reproduce(self, pop_density):
        ...
        if random.random() <= self.birth_prob * (1 - pop_density):
            if self.get_resistant():
                ...
                possible_resistance = random.random() <= self.mut_prob * (1-pop_density)
                return ResistantBacteria(self.birth_prob, self.death_prob, possible_resistance, self.mut_prob)
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        if self.on_antibiotic:
            num_surviving = len(surviving_bacteria)
            for i in range(num_surviving):
                try:
                    if not surviving_bacteria[i].get_resistant():
        ...
        for i in range(len(self.bacteria)):
            if self.bacteria[i].is_killed() == False:
                bac_postreap.append(self.bacteria[i])

        self.bacteria = bac_postreap
        ...
        if self.on_antibiotic == True:
            ...
            for i in range(len(self.bacteria)):
                if self.bacteria[i].get_resistant() == True:
                    bac_postab.append(self.bacteria[i])
            self.bacteria = bac_postab
        ...
        for i in range(len(self.bacteria)):
            ...
            new_bac = self.bacteria[i].reproduce(pop_density) 
...
def simulation_with_antibiotic(num_bacteria,
    ...
    for i in range(num_trials):
        ...
        for j in range(1, no_ab_timesteps):
            populations[i].append(patient.update())
            resistant_pop[i].append(patient.get_resist_pop())

        patient.set_on_antibiotic()

        for j in range(no_ab_timesteps, no_ab_timesteps + with_ab_timestep):
            populations[i].append(patient.update())
            resistant_pop[i].append(patient.get_resist_pop())

    
    x_vals = []
    for i in range(no_ab_timesteps + with_ab_timestep):
        x_vals.append(i)
        average_pop.append(calc_pop_avg(populations, i))  
        resistant_pop_avg.append(calc_pop_avg(resistant_pop, i))   
    
    #make_one_curve_plot(x_vals, average_pop, 'Timestep', 'Average Population', 'Without Antibiotic')
    make_two_curve_plot(x_vals, average_pop, resistant_pop_avg, 'Total', 'Resistant', 'Timestep', 'Average Population', 'With an Antibiotic')
